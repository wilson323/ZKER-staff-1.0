...
verify('Four browser suites passed all actual cases',all(r['passed'] and all(c['passed'] for c in r['checks']) for r in reports))
verify('Role, OA and field browser suites match current HTML digest',all(r['htmlSha256']==sha(html) for r in [reports[0],reports[2],reports[3]]))
...
verify('Current UI version visible', '交互设计原型 v2.6' in html.read_text() and 'OA 多事务' in html.read_text())
links=[];bad=[]
...
extra=json.loads((root/'24-业务模板字段候选.json').read_text());verify('17 business template fields match embedded definitions',sum(map(len,extra['templates'].values()))==17 and json.loads(re.search(r'var oaFormDefinitions=(.*?);\nvar oaTemplates=',html.read_text(),re.S)[1])==extra['templates']);verify('Existing contract field sources unchanged',json.loads((root/'validation/v26-contract-field-source-audit.json').read_text())['unchanged']==64);verify('All roles have field inventory without full-contract claim',reports[3]['roleCount']==41 and all(x['semanticStatus']=='INVENTORIED_NOT_FULL_CONTRACT_ACCEPTANCE' for x in reports[3]['inventory']));coverage.update(fieldInventoryViews=reports[3]['inventoryViews'],businessTemplateFields=17)
result=dict(revision='v2.6',passed=all(c['passed'] for c in checks),checks=checks,coverage=coverage,localLinks=len(links),htmlSha256=sha(html),sourceHead=subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True).strip(),sourceHashes=sourceHashes,limitations=['Local fictional prototype, schema and documentation only; no production database, real-role authentication, cross-browser concurrency, calendar engine or actual Agent runtime verification.'])
(root/'validation/v26-document-validation.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n');print(json.dumps(dict(passed=result['passed'],checks=len(checks),coverage=coverage,localLinks=len(links),failures=[x for x in checks if not x['passed']]),ensure_ascii=False));sys.exit(0 if result['passed'] else 1)
